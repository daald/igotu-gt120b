  ...
  pub fn new() -> Self {
    ...
    for line in read_to_string("src/replay-120b.txt").unwrap().lines() {
        ...
        if line.starts_with("#"){
            ...
            result.push(InOut{out: true, line: hex::decode(line.to_string()[2..].replace(":", "")).expect("Decoding failed"), line_num: line_num});
        } else if line.starts_with("< "){
            result.push(InOut{out: false, line: hex::decode(line.to_string()[2..].replace(":", "")).expect("Decoding failed"), line_num: line_num});
        } else {
...
impl Intf for IntfFile {
  fn send_and_receive(&mut self, to_device: Vec<u8>) -> Vec<u8> {
    ...
    if out_line.line != to_device {
      panic!("Next cmd doesn't match: #{}\nactual: {:02X?}\nexpected: {:02X?}", out_line.line_num, out_line.line, to_device);
    }
